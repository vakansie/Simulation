...
class Simulation():
    ...
    def bind_keys(self):
        ...
        self.window.bind("x", lambda x: self.toggle_random())
...
class Spreader(Forces):
    def iterate(array_x_pos, array_y_pos):
        ...
        for i in range(simulation.spreading_factor):
            try: x_direction, y_direction = simulation.direction_options[(random_index - i if simulation.random else simulation.spin - i)]
